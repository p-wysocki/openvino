...
TEST_F(TaskSystemTests, ClaimsOneHundredTasks) {
  ...
  cl_int status = CL_SUCCESS;
  cl_mem claimedTaskIds = clCreateBuffer(
  ...
  std::vector<int> actualTaskIds(taskCount, -1);
  ASSERT_OCL_SUCCESS(clEnqueueWriteBuffer(
      queue(), claimedTaskIds, CL_TRUE, 0, taskCount * sizeof(int),
      actualTaskIds.data(), 0, nullptr, nullptr));
  ...
  ASSERT_OCL_SUCCESS(
      clSetKernelArg(binary.kernel, 1, sizeof(cl_mem), &claimedTaskIds));
  ...
  ASSERT_OCL_SUCCESS(clReleaseMemObject(claimedTaskIds));
