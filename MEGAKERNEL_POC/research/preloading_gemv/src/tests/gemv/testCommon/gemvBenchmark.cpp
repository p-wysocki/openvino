...
cl_int enqueueGemvKernel(cl_mem inputBuffer, cl_mem matrixBuffer,
                         cl_mem outputBuffer, const GemvParams& shape,
  ...
  for (size_t layer = 0; layer < shapes.size(); ++layer) {
    ...
    oclBinaries[layer] = createProgramAndKernel(
        ...
        "-cl-std=CL3.0 -I " + kernelSourcePath + " -I " +
            ...
            " -DGEMV_PHASE_TILE_ROWS=" +
            std::to_string(shape.gemvPhaseTileRows) +
            " -DGEMV_COMPUTE_WARPS=" + std::to_string(shape.gemvComputeWarps));
